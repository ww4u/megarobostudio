...
int  Test(int length, int pStep, int pMode)
{
	return 1;
}
...
//!
int  GetPvtInfo(double** pIn, int length,
                double pStep,
                int pMode,
                double* resOut )
{
	...
	CallRobot(length,resOut);

	return 1;
}

//! len -- data in length in cell unit
//!
void CallRobot(int len,double* resOut)
{
	// 计算点的个数
	int count = len / 6;
	int pcount = 0;
	...
	for (int i = 0; i < count - 1; i++)
	{
		...
		{
			pos[j].x = posIn[0][6 *j];
			pos[j].y = posIn[0][6 *j+1];
			pos[j].z = posIn[0][6 * j+2];
			pos[j].t = posIn[0][6 * j + 4];
			pos[j].m = posIn[0][6 * j + 5];
		}
		// 计算两点间的空间距离
		...
		{
			res.Px = tempP[j] * delt[0] + posIn[0][0];
			res.Py = tempP[j] * delt[1] + posIn[0][1];
			res.Pz = tempP[j] * delt[2] + posIn[0][2];
			res.Vx = tempV[j] * delt[0];
			res.Vy = tempV[j] * delt[1];
			res.Vz = tempV[j] * delt[2];
			res.End = posIn[i][(i + j) * 5 + 3];
			res.T = tempT[j];
			resInfo.push_back(res);
		}
	}
	...
	{
        resOut[m * 6 + 0] = resInfo[m].Px;
        resOut[m * 6 + 1] = resInfo[m].Py;
		resOut[m * 6 + 2] = resInfo[m].Pz;
		resOut[m * 6 + 3] = resInfo[m].Vx;
		resOut[m * 6 + 4] = resInfo[m].Vx;
		resOut[m * 6 + 5] = resInfo[m].Vx;
		resOut[m * 6 + 6] = resInfo[m].T;
	}
}
...
double* DivRegion(double* pIn,int plen, double* tIn, int* len)
{
	...
	double delt1 = fabs(CallPolynomial(dConst, plen - 1, tIn[0]));
	double delt2 = fabs(CallPolynomial(dConst, plen - 1, tIn[1]));
	// 边界导数为0，不需要划分区间
	...
	{
		delt1 = fabs(tIn[0] - sol);
		delt2 = fabs(tIn[1] - sol);
		if (delt1 < error || delt2 < error)
		{
			len[0] = 2;
			res = tIn;
...
double SolveQuadraticFunction(double* pIn, double* tIn)
{
	...
	double c = pIn[2];
	double res = 0;
	if (fabs(a) < error || fabs(a / b) < error)
	{
	...
	else
	{
		...
		else
		{
			...
			if (fabs(pIn[1] - pIn[0]) < error)
			{
				for (int i = 0; i < 2; i++)
				{
					if (fabs(tIn[0] - x[i]) < error)
					{
						res = tIn[0];
					}
				}
...
{
	//区间为一点
	if (fabs(tIn[1] - tIn[0]) < error)
	{
		if (fabs(tIn[0] - res) < error)
		{
			return res;
		}
	}
