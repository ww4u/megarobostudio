...
void GetNewPvt(double* pIn, double*vIn, double*tIn, int length, int mode, double* pOut, double* vOut, double*tOut, int* pLen)
{
	...
	for (int i = 0; i < length; i++)
	{
		if (i < length - 1)
		{
			...
			{
				pvtNum = pvtNum + 2;
                                double* pZcp = (double*)malloc(joinNum*pvtNum * sizeof(double) * 2 );
                                double* vZcp = (double*)malloc(joinNum*pvtNum * sizeof(double) * 2 );
                                double* tZcp = (double*)malloc(pvtNum * sizeof(double) * 2 );
                                memset(pZcp, 0, joinNum*pvtNum * sizeof(double) * 2 );
                                memset(vZcp, 0, joinNum*pvtNum * sizeof(double) * 2 );
                                memset(tZcp, 0, pvtNum * sizeof(double) * 2 );
...
void Rank(double* input,int len)
{
	for (int i = 0; i < len-1; i++)
	{
		...
		{
			if (input[len +i] > input[len +j])
			{
				double temp1 = input[len + i];
				input[len + i] = input[len + j];
				input[len + j] = temp1;
